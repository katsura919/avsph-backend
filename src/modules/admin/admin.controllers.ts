...
export async function createAdmin(
  ...
  const newAdmin = {
    ...
    role: isFirstAdmin
      ...
      : parseResult.data.role || ("admin" as const),
  ...
  if (assignedBusinessIds.length > 0) {
    ...
    if (businesses) {
      await businesses.updateMany(
        { _id: { $in: assignedBusinessIds.map((id) => new ObjectId(id)) } },
...
export async function updateAdmin(
  ...
  if (parseResult.data.businessIds !== undefined) {
    ...
    if (businesses) {
      ...
      const removedBusinessIds = oldBusinessIds.filter(
        (bid: string) => !newBusinessIds.includes(bid),
      );
      if (removedBusinessIds.length > 0) {
        await businesses.updateMany(
          {
            _id: {
              $in: removedBusinessIds.map((bid: string) => new ObjectId(bid)),
            },
          },
          { $pull: { adminIds: id } as any },
        );
      }

      // Add admin to newly assigned businesses
      const addedBusinessIds = newBusinessIds.filter(
        (bid: string) => !oldBusinessIds.includes(bid),
      );
      if (addedBusinessIds.length > 0) {
        await businesses.updateMany(
          {
            _id: {
              $in: addedBusinessIds.map((bid: string) => new ObjectId(bid)),
            },
          },
