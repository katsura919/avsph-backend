...
export async function getAttendanceByBusiness(
    request: FastifyRequest<{ Params: BusinessIdParams; Querystring: AttendanceQuery }>,
    reply: FastifyReply,
) {
    const attendance = request.server.mongo.db?.collection("attendance");
    const businesses = request.server.mongo.db?.collection("businesses");
...
export async function getAttendanceByStaff(
    request: FastifyRequest<{ Params: StaffIdParams; Querystring: AttendanceQuery }>,
    reply: FastifyReply,
) {
    const attendance = request.server.mongo.db?.collection("attendance");
    const staff = request.server.mongo.db?.collection("staff");
